...
def build_dataset(rows, processor):
    ...
    for r in rows:
        ...
        labels = processor.tokenizer((r["transcript"] or "").strip()).input_ids
...
def main():
    ...
    train_rows = load_csv_manifest(cfg["dataset"]["train_manifest"])
    val_rows = load_csv_manifest(cfg["dataset"]["val_manifest"])
    ...
    trainer.train()
    best_dir = os.path.join(tcfg["output_dir"], "best")
